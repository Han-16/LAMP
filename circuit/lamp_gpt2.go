...
func (c *LAMPGPT2Circuit) Define(api frontend.API) error {
	...
	// Bind every folded RS message and codeword before using one shared challenge.
	rsWitness := make([]frontend.Variable, 0)
	for i := range c.Claims {
		rsWitness = append(rsWitness, AppendVariables(
			c.Claims[i].VecX,
			c.Claims[i].VecYZ,
			c.Claims[i].VecBTest,
			c.Claims[i].EncX,
	...
	for i := range c.Claims {
		...
		if err := c.defineClaim(api, &h, &c.Claims[i], challengeBPowers, rsPoint); err != nil {
			return err
		}
	}

	for i := range c.RSBatches {
		if err := c.defineRSBatch(api, &c.RSBatches[i], rsPoint); err != nil {
			return err
		}
	}
...
func (c *LAMPGPT2Circuit) defineClaim(api frontend.API, h hash.FieldHasher, claim *LAMPGPT2RectClaim, challengeBPowers []frontend.Variable, rsPoint frontend.Variable) error {
	h.Reset()
	...
	if !claim.SkipRSX {
		VerifyRSEncoding(api, claim.Inner, claim.NIn, claim.DomainKIn, claim.WeightsKIn, claim.DomainNIn, claim.WeightsNIn, claim.VecX, claim.EncX, rsPoint)
	}
	if !claim.SkipRSYZ {
		VerifyRSEncoding(api, claim.Cols, claim.NOut, claim.DomainKOut, claim.WeightsKOut, claim.DomainNOut, claim.WeightsNOut, claim.VecYZ, claim.EncYZ, rsPoint)
	}
...
func (c *LAMPGPT2Circuit) defineRSBatch(api frontend.API, batch *LAMPGPT2RSBatch, rsPoint frontend.Variable) error {
	if len(batch.Terms) == 0 {
		return fmt.Errorf("empty LAMP GPT-2 RS batch %d", batch.ID)
	}

	betaPowers := Powers(api, rsPoint, len(batch.Terms))
	...
	VerifyRSEncoding(api, batch.K, batch.N, batch.DomainK, batch.WeightsK, batch.DomainN, batch.WeightsN, combinedVec, combinedEnc, rsPoint)
	return nil
}
