...
func InitFreivaldsCSV(filename string) (*os.File, *csv.Writer) {
	...
	writer := csv.NewWriter(file)
	header := []string{"log(K)", "Compute (s)", "Constraint", "Setup (s)", "Prove (s)", "Verify (s)"}
...
func AppendFreivaldsResultToCSV(writer *csv.Writer, res FreivaldsResult) {
	record := []string{
		...
		strconv.Itoa(res.Constraints),
		fmt.Sprintf("%.3f", res.Setup),
		fmt.Sprintf("%.3f", res.Prove),
		fmt.Sprintf("%.3f", res.Verify),
	}
	...
	if err := writer.Error(); err != nil {
		...
		fmt.Printf("💾 Result saved to CSV: logK=%d\n", res.LogK)
	}
}
