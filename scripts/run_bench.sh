...
usage() {
	...
	echo "  scripts/run_bench.sh lamp_batch --K 10 --L 128 --batch 5" >&2
	echo "  scripts/run_bench.sh lamp_batch --K 10 --L 128 --batch-range --batch-from 1 --batch-to 10 --compile" >&2
	echo "  scripts/run_bench.sh lamp --K 10 --L 128 --merkle multi" >&2
	echo "  scripts/run_bench.sh freivalds --K 10 --compile" >&2
	echo "  scripts/run_bench.sh freivalds_batch --K 7 --batch 5 --compile" >&2
...
for key in \
	...
	LAMP_BATCH_LOG_K LAMP_BATCH_RHO LAMP_BATCH_L LAMP_BATCH_SIZE LAMP_BATCH_MERKLE LAMP_BATCH_ALL LAMP_BATCH_RANGE LAMP_BATCH_ONLY_COMPILE LAMP_BATCH_OUTPUT_DIR LAMP_BATCH_LOG_K_FROM LAMP_BATCH_LOG_K_TO LAMP_BATCH_FROM LAMP_BATCH_TO \
