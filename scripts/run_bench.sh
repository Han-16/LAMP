...
usage() {
	echo "usage: scripts/run_bench.sh [meow|freivalds|gpt2|all] [--range] [flags...]" >&2
	echo "" >&2
	echo "examples:" >&2
	echo "  scripts/run_bench.sh meow --K 10 --L 128" >&2
	echo "  scripts/run_bench.sh freivalds --K 10 --compile" >&2
	echo "  scripts/run_bench.sh gpt2 --protocol meow --model small --seq_len 1024" >&2
...
for key in \
	MEOW_LOG_K MEOW_RHO MEOW_L MEOW_ALL MEOW_ONLY_COMPILE MEOW_OUTPUT_DIR MEOW_LOG_K_FROM MEOW_LOG_K_TO \
	FREIVALDS_LOG_K FREIVALDS_ALL FREIVALDS_ONLY_COMPILE FREIVALDS_OUTPUT_DIR FREIVALDS_LOG_K_FROM FREIVALDS_LOG_K_TO \
	GPT2_PROTOCOL GPT2_MODEL GPT2_SEQ_LEN GPT2_L GPT2_RHO GPT2_ONLY_COMPILE GPT2_OUTPUT_DIR
do
